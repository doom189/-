...
def ShangXiaBan(员工工号, 日期, 刷卡时间):
    KeyStr = f"{员工工号}丨{日期}"  # 工号+日期
    if KeyStr in ShuaKaData:
        BanCi = RenYuanBanCi[员工工号]  # 取得班次名
        if BanCi == '两班倒':
            if GetVal(ShuaKaData[KeyStr], "1下班") == '':
                ShuaKaData[KeyStr]["1下班"] = 刷卡时间
                return
            ShuaKaData[KeyStr]["2上班"] = 刷卡时间
        if BanCi == '责任制3笔':
            ShuaKaData[KeyStr]["下午上班"] = 刷卡时间
            return  # 结束
        if GetVal(ShuaKaData[KeyStr], "上午下班") == '':
            ShuaKaData[KeyStr]["上午下班"] = 刷卡时间
            return
        ShuaKaData[KeyStr]["下午上班"] = 刷卡时间
...
# print(BanCiDic)  # 输出班次信息数据结构
# print(RenYuanBanCi)  # 输出人员班次信息数据结构
wb.close()
app.quit()  # 退出
    ...
    for name in files:
        if re.match(r'.*?\d{1,2}月打卡记录\.xlsx?$', name, re.M):
            ...
            for row in 读取数据:
                ...
                #region 自动两班倒
                if BanCi == '两班倒':
                    if ShiJian >= 18 and ShiJian <= 20:
                        ShiDuan = '1'  # 上班
                        BanCi = '生产夜班'
                    elif ShiJian > 0 and ShiJian < 10:
                        ShiDuan = '智能中间判断'
                ...
                KeyStr = f"{YGGH}丨{ShuaKaRiQi}"  # 员工工号+日期 = 数据键
                # 构建工号+日期数据结构
                if KeyStr not in ShuaKaData:
                    ShuaKaData[KeyStr] = {
                        "日期": ShuaKaRiQi,
                        "员工工号": YGGH,
                        "员工姓名": YGXM
                ...
                if WeekNum > 5:
                    ...
                    # 取第一笔打卡记录
                    if ShiDuan + "上班" not in ShuaKaData[KeyStr]:
                        ShuaKaData[KeyStr][ShiDuan + "上班"] = ShuaKa
                    else:
                        ShuaKaData[KeyStr][ShiDuan + "下班"] = ShuaKa

                        # 计算加班时长
                        JiaBan = JiSuanJiaBan(ShuaKaShiJian, Str2Time(ShuaKaData[KeyStr][ShiDuan + "上班"]))
                        if JiaBan > 0:
                            ShuaKaData[KeyStr]["加班"] = GetFloat(ShuaKaData[KeyStr], "加班") + JiaBan
                        # 计算加班时长 尾巴
                    # 取第一笔打卡记录 尾巴
                    continue  # 跳过本行
                # 判断是否是周六日 尾巴
                    ...
                    if ShiDuan in BanCiDic[BanCi]:
                        ...
                        # 例外情况测试 1. 迟到; 2. 请假; 3. 漏打卡;
                            ...
                            if ShuaKaShiJian > Str2Time(BanCiDic[BanCi][ShiDuan]['上班时间']) \
                                    ...
                                    and ShiDuan + "上班" not in ShuaKaData[KeyStr]:
                                # print("上班迟到咯! 如果请假请自行处理")
                                ShuaKaData[KeyStr][ShiDuan + "上班"] = f"{ShuaKa}丨迟到"
                                continue  # 迟到跳出
                            # 迟到 尾巴
                            ...
                            if ShuaKaShiJian > Str2Time(BanCiDic[BanCi][ShiDuan]['上班时间']) \
                                    and ShuaKaShiJian < XiaBanShiJian \
                                    and ShiDuan + "下班" not in ShuaKaData[KeyStr]:
                                # print("早退, 如果请假请自行处理")
                                ShuaKaData[KeyStr][ShiDuan + "下班"] = f"{ShuaKa}丨早退"
                                continue  # 早退跳出
                            # 早退 尾巴
